...
@section('content')
    ...
    <form class="login100-form validate-form" method="POST" action="{{url('signin')}}">
        ...
        <div>
            @if (session('success'))
                <span class="alert alert-success" style="color: green;">{{ session('success') }}</span><br/><br/><br/>
            @endif
            @if (session('status'))
                <span class="alert alert-danger" style="color: red;">{{ session('status') }}</span><br/><br/><br/>
            @endif
        </div>
